...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm,               xK_r     ), spawn "/home/zhou/bin/dmenu_wrap_yeganesh")
...
--    , ((0 ,                0x1008ff13), spawn "ossmix vmix0-outvol -- +1")
    , ((0 ,                0x1008ff2f), spawn "gksudo s2ram")
    ...
    , ((modm .|. shiftMask, xK_r     ), spawn "killall conky dzen2 tint2 parcellite nm-applet fcitx volumeicon; xmonad --recompile; xmonad --restart")
    ]
    ++
...
--    , [className =? s  --> doTile   | s <- myTiles]
    ...
    where
    myFloats      = ["Gpick", "Chromium", "Gimp", "Volumeicon", "MPlayer", "Smplayer2", "Gnome-mplayer", "Tintwizard", "Vlc", "Lxrandr", "Arandr", "Audacious", "Deadbeef", "VirtualBox", "Firefox", "Firefox-bin", "AliWangWang", "Q4wine", "Wine", "Linux-fetion", "Gcalctool", "Gmlive", "Skype", "Ossxmix", "Pidgin", "Emesene", "Openfetion", "Vncviewer", "Cairo-dock", "RipperX", "Goldendict", "PBurn" ]
    ...
    myIgnores     = ["trayer", "dzen", "stalonetray", "systray", "cairo-dock", "Wine"]
...
--                              "SimplestFloat"            -> wrap "^ca(1,xdotool key super+space)" "^ca()" ("^i(" ++ myIconDir ++ "floating.xbm)")
--                              "magnifier"              -> "^i(/home/zhou/.xmonad/dzen/layouts/magnifier.xbm)"
        ...
        , ppTitle    = ("^fn(SimSun-9)" ++) . dzenColor dzFgColor dzBgColor . dzenEscape
        , ppOutput   = hPutStrLn h
      }
      where
        rangeId wsName cornerColor bgColor fgColor = let base=drop 2 wsName; corner=take 1 wsName; getid1=take 2 wsName; getid=drop 1 getid1 in "^ca(1,xdotool key super+" ++ getid ++ ")^fg(" ++ fgColor ++ ")^bg(" ++ bgColor ++ ") " ++ base ++ "^p(;-2)^fn(" ++ mySmallFont ++ ")^fg(" ++ cornerColor ++ ")" ++ corner ++ " ^fn()^p()^fg()^bg()^ca()" 

-- myWorkspaces = ["1:Term", "2:Surf", "3:Edit", "4:File", "5:View", "6:Pics"]
myWorkspaces = ["Q1Surf", "I2Wine", "U3Edit", "T4File", "J5View", "u6Pics"]

--
-- Layouts
genericLayout = tiled ||| Mirror tiled  ||| Full
  where
